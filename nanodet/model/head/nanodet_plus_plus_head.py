...
class AsymmetricDecoupleConv(nn.Module):
    def __init__(self,in_dim,
                        cls_out_dim,
                        reg_out_dim,
                        activation=None,
                ):
        super(AsymmetricDecoupleConv, self).__init__()
        ...
        self.reg_conv = nn.Sequential(
                ...
                activation(inplace=True),#nn.LeakyReLU(),
...
class NanoDetPlusPlusHead(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(NanoDetPlusPlusHead, self).__init__()
    ...
    def _init_layers(self):
        ...
        self.gfl_cls = nn.ModuleList(
            [AsymmetricDecoupleConv(self.feat_channels,self.num_classes,4*(self.reg_max+1),activation=activations[self.activation]) for _ in self.strides
            ]
        )
    ...
    def init_weights(self):
        ...
        for i in range(len(self.strides)):
            for m in self.gfl_cls[i].modules():
                if isinstance(m, nn.Conv2d):
