...
def quality_focal_loss(pred, target, beta=2.0):
    ...
    #loss_extra = extra_loss(pred_sigmoid, target)
...
def extra_loss(pred, target):
    ...
    for i in range(low,high):
        if  label0[i] not in allC:
            ...
            loss = -(scorc-scorc[index])*torch.log( torch.min(torch.max(1+predc-scorc ,torch.tensor(0.1,device='cuda')),torch.tensor(1,device='cuda')))
            #loss = loss[loss>=0]
            Loss[idx] = loss
    #bol = Loss < 0
    #Loss[bol] = 0
    ...
    return 0.1*torch.mean(Loss[labelBool])