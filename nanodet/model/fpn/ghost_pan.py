...
from ..backbone.ghostnet import GhostBottleneck,GhostBottleneckPlus
...
class GhostBlocks(nn.Module):
    ...
    def __init__(
        ...
        ghost_block_type = 'normal',
    ):
        super(GhostBlocks, self).__init__()
        #print(ghost_block_type,'^^^^^^^^^')
        ...
        for _ in range(num_blocks):
            blocks.append(
                GhostBottleneck(
                    ...
                    activation=activation,
                ) if ghost_block_type == 'normal' else GhostBottleneckPlus( in_channels,
                    int(out_channels * expand),
                    out_channels,
                    dw_kernel_size=kernel_size,
                    activation=activation,)
        ...
        for idx in range(len(in_channels) - 1, 0, -1):
            self.top_down_blocks.append(
                GhostBlocks(
                    ...
                    use_res=use_res,
                    activation=activation,
                    ghost_block_type=ghost_block_type,
                )
            )
